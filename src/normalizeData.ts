...
interface INormalizeDataOutput {
  byId: IById
  allIds: string[]
}

const normalizeData = (unnormalizedData: IPost[]): INormalizeDataOutput => {
  const normalizeData: INormalizeDataOutput = {
    byId: {},
    allIds: []
  }

  return unnormalizedData.reduce((data, post) => {
    return {
      ...data,
      byId: {
        ...data.byId,
